...
export class PrivilegeService {
  ...
  public async getOnlySessionEndpoints(
    ...
    return (await this.privilegeEndpointsEntityRepository.query(
      "SELECT DISTINCT e.method, e.uri, e.privilege_slug FROM store.privilege_endpoints e left JOIN store.role_privileges r ON r.privilege_slug = e.privilege_slug JOIN store.roles g on r.role_id = g.id WHERE  e.uri_mode = 'ONLY_SESSION' AND g.name = ANY($1) AND e.method = $2 AND e.uri = $3",
      [roles, method, uri],
    )) as EndpointDto[];
  }
}
